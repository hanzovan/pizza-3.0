...
export default function CategoriesPage() {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    await toast.promise(creatingPromise, {
      ...
      success: editCategory
        ? "Updating category successfully"
  ...
  const fetchCategories = () => {
    fetch("/api/categories").then((response) =>
      response.json().then((fetchedCategories) => {
        ...
        <form onSubmit={handleFormSubmit}>
          <div className="flex items-end gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              {editCategory && (
                <button
                  className="bg-gray-200 border border-gray-300 px-8"
                  type="button"
                  onClick={() => {
        ...
        {categories && (
          <div>
            ...
            {categories.map((c, index) => (
              <div
                key={index}
                className="flex items-center bg-gray-200 gap-1 my-2 rounded-xl px-4 py-2"
              >
                <div className="grow">{c.name}</div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    className="border border-gray-400 bg-gray-300"
                    onClick={() => {
                      setEditCategory(c);
