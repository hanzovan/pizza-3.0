        ...
        <div className="md:max-w-3xl sm:max-w-lg max-w-sm mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 md:gap-10 gap-12 place-items-center">
                {menuItems.map((item, index) => (
                    <Link key={index} href={"/menu-items/" + item._id + "/edit"} className="px-12 text-center bg-gray-300 p-4 rounded-lg flex flex-col my-2 justify-between items-center duration-300 hover:scale-105 hover:bg-gray-100">
                        <div>
                            <Image src={item.image || ""} alt={item.name || ""} width={100} height={100} className="w-48 h-48 sm:w-36 sm:h-36 md:w-28 md:h-28" />
                        </div>
